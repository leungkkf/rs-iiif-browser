...
#[serde(untagged)]
pub(crate) enum PresentationInfo {
    Version2(presentation_v2::PresentationInfo),
    Version3(presentation_v3::PresentationInfo),
}
...
impl PresentationInfo {
    ...
    fn from_json(info_json: &str) -> core::result::Result<Self, IiifError> {
        let iiif_presentation_info: PresentationInfo = serde_json::from_str(info_json)?;
        debug!("iiif_image_info {:?}", iiif_presentation_info);

        Ok(iiif_presentation_info)
    }
}
...
mod tests {
    ...
    fn test_from_json() {
        ...
        match PresentationInfo::from_json(&json) {
            Ok(PresentationInfo::Version2(_)) => assert!(true),
            _ => assert!(false),
        }
        ...
        match PresentationInfo::from_json(&json) {
            Ok(PresentationInfo::Version3(_)) => assert!(true),
            _ => assert!(false),
        }
    }
}
