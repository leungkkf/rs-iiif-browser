...
impl IsProfileDetails for IiifProfileDetails {
    fn get_supported_features(&self) -> impl ExactSizeIterator<Item = IiifFeature> {
        self.supports.iter().map(|x| x.to_owned())
    }

    fn get_formats(&self) -> impl ExactSizeIterator<Item = IiifImageFormat> {
        self.formats.iter().map(|x| x.to_owned())
    }
}
...
impl IiifImageInfo {
    /// Build from a URL.
    pub(crate) fn try_from_url(url: &str) -> core::result::Result<impl IsImage, IiifError> {
    ...
    fn try_from_json(info_json: &str) -> core::result::Result<Self, IiifError> {
...
impl IsImage for IiifImageInfo {
...
mod tests {
    ...
    fn test_from_json() {
        ...
        let image_info = IiifImageInfo::try_from_json(&json).unwrap();
